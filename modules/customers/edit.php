...
if (isset($_POST['update'])) {
    ...
    elseif ($email !== '' && !filter_var($email, FILTER_VALIDATE_EMAIL)) { $error = 'Please enter a valid email address.'; }
        ...
        $statement = mysqli_prepare($conn, 'UPDATE customers SET name = ?, phone = ?, email = ?, address = ? WHERE id = ?');
        mysqli_stmt_bind_param($statement, 'ssssi', $name, $phone, $email, $address, $id);
        mysqli_stmt_execute($statement);
        header('Location: index.php?success=Customer updated successfully.'); exit;
    }
    $customer = array_merge($customer, ['name' => $name, 'phone' => $phone, 'email' => $email, 'address' => $address]);
}
    ...
    <div class="rm-modal-body">
        ...
        <form method="POST">
            ...
            <div class="mb-4">
                ...
                <textarea name="address" class="form-control rm-input" rows="3" style="height:auto;">
